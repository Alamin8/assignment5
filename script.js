    ...
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${foodName}`)
        ...
        .then(data => {
            ...
            data.meals.forEach(element => {
                ...
                food.innerHTML = `
            <img src="${element.strMealThumb}" onClick="foodClick(${element.idMeal})">
            <h1 onClick="foodClick(${element.idMeal})" >${element.strMeal}</h1>
            `;
                food.className = "card";
        ...
        .catch(error => {
            ...
            notFound.innerHTML = `Sorry, this food not found...Try again!`;
            foods.appendChild(notFound);
        })
})

let foodClick = foodId => {
    fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${foodId}`)
        .then(res => res.json())
        .then(data => {
            let foodData = document.getElementById('foodData');
            document.getElementById('foodData').innerHTML = ' ';
            document.getElementById('foodData').style.display = 'block';
